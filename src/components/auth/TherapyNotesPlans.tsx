...
interface TherapyNotesPlansProps {
  plans: Plan[];
  onSelectPlan: (planId: string) => void;
}

export function TherapyNotesPlans({ plans, onSelectPlan }: TherapyNotesPlansProps) {
  return (
    <div>
      <h3 className="text-xl font-semibold mb-6">Therapy Notes Plans</h3>
      <div className="grid md:grid-cols-3 gap-8">
        {plans.map((plan) => (
          <Card 
            key={plan.id} 
            className={`p-6 flex flex-col ${plan.name === 'Professional' ? 'border-2 border-primary relative' : ''}`}
          >
